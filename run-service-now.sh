...
# Activate the appropriate environment based on ENV setting
if [ "$ENV" = "draft" ]; then
    echo "Activating local environment for draft mode..."
...
if [ -n "$SERVICE_NOW_URL" ]; then
    ...
    orchestrate connections configure -a service-now --env "${ENV:-live}" --type team --kind basic --url "$SERVICE_NOW_URL"
else
    echo "Warning: SERVICE_NOW_URL not found in .env file"
    echo "Please provide the ServiceNow instance URL when prompted"
    orchestrate connections configure -a service-now --env "${ENV:-live}" --type team --kind basic
fi
...
if [ -n "$SERVICE_NOW_PASSWORD" ]; then
    ...
    orchestrate connections set-credentials -a service-now --env "${ENV:-live}" -u admin -p "$SERVICE_NOW_PASSWORD"
else
    echo "Warning: SERVICE_NOW_PASSWORD not found in .env file"
    echo "Please provide the ServiceNow password when prompted"
    orchestrate connections set-credentials -a service-now --env "${ENV:-live}" -u admin
fi
